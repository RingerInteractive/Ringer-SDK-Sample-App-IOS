...
extension ProgressHUDDelegate {
 ...
 class ProgressHUD: UIView {
    ...
    static let maxOffset: CGFloat = 1000000.0
    ...
    // public
     ...
     var bezelView: ProgressHUDBackgroundView?
     var backgroundView: ProgressHUDBackgroundView?
     ...
     var mode: HudMode = .indeterminate {
        didSet {
            if(mode != oldValue) {
                updateIndicators()
            }
     ...
     var contentColor = UIColor(white: 0, alpha: 0.7) {
        didSet {
    ...
    var animationType: HudAnimation = .fade
    var offset: CGPoint = CGPoint(x: 0, y: 0)
    var margin: CGFloat = 20.0
    var minSize:CGSize = CGSize.zero
    var isSquare = false         // force the hud dimensions to be equal if possible
    var isDefaultMotionEffectsEnabled = true
    var minShowTime: TimeInterval = 0.0
    var completionBlock: (() -> Void)?
    var delegate: ProgressHUDDelegate?
    ...
    var progressObjectDisplayLink: CADisplayLink? {
     ...
     class func show(addedToView view: UIView, animated: Bool) -> ProgressHUD {
        let hud = ProgressHUD(withView: view)
     ...
     class func hide(addedToView view: UIView, animated: Bool) -> Bool {
