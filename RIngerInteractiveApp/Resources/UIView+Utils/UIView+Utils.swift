...
extension UIView {
    ...
    @IBInspectable var bottomBorder: Bool {
        ...
        set {
            if newValue {
                let border = CALayer()
                border.backgroundColor = #colorLiteral(red: 0.8823529412, green: 0.8901960784, blue: 0.8980392157, alpha: 1)
                border.frame = CGRect(x: 0, y: self.frame.size.height, width: self.frame.size.width + 20, height: 1)
    ...
    @IBInspectable var setBottomCornerRadious: CGFloat {
        get {
            return 0
        }
        set {
...
extension UIViewController {
    func showLoginDialog() {
        let alert = UIAlertController(title: "Login required!", message: "Please Login to continue", preferredStyle: UIAlertController.Style.alert)
        alert.addAction(UIAlertAction(title: "Log in", style: UIAlertAction.Style.default, handler: { _ in
...
extension UIViewController {
    ...
    func multiOptionAlertBox(title: String, message: String, action1: String, action2: String = "", completion: @escaping (_ status: Int) -> Void) {
        let alertController = UIAlertController(title: title, message: message, preferredStyle: .alert)
        alertController.addAction(UIAlertAction(title: action1, style: .default, handler: { (_ ) in
            completion(0)
        }))
        if action2 != "" {
            alertController.addAction(UIAlertAction(title: action2, style: .default, handler: { (_ ) in
                completion(1)
            }))
        }
        self.present(alertController, animated: true, completion: nil)
...
extension UIViewController: UITextFieldDelegate {
    public func textFieldShouldBeginEditing(_ textField: UITextField) -> Bool {
        textField.borderColor = UIColor(named: "themeColor") ?? .black
        return true
    }
    public func textFieldDidEndEditing(_ textField: UITextField) {
        textField.borderColor = UIColor(named: "normalFieldBorder") ?? .black
    }
    public func isValidData(textField: UITextField) -> Bool {
...
extension NSMutableAttributedString {
    ...
    var boldFont:UIFont { return UIFont(name: "CondellBio-SemiBold", size: fontSize) ?? UIFont.boldSystemFont(ofSize: fontSize) }
    var normalFont:UIFont { return UIFont(name: "CondellBio-Medium", size: fontSize) ?? UIFont.systemFont(ofSize: fontSize)}
